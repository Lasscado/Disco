...
class Owner(commands.Cog):
    ...
    async def _fetch_logs_channels(self):
       self.ban_logs = await self.lite.fetch_channel(int(os.environ['GLOBAL_BANS_CHANNEL_ID']))
    ...
    async def _global_ban(self, ctx, target_type, target_id: int, *, reason):
        ...
        if self.lite._bans.find(targetID=target_id, ignore=False):
            return await ctx.send(f'{self.lite.emoji["false"]} **{ctx.author.name}**, esse alvo '
                'já está banido do meu sistema.')

        if target_type in ['user', 'u']:
            try:
                target = await self.lite.fetch_user(target_id)
            except discord.NotFound:
                return await ctx.message.add_reaction(self.lite.emoji["false"])

            self.lite.user_blacklist.add(target.id)

            icon = target.avatar_url
            target_type = 'Usuário'
        elif target_type in ['server', 'guild', 'g']:
            try:
                target = await self.lite.fetch_guild(target_id)
            except discord.NotFound:
                return await ctx.message.add_reaction(self.lite.emoji["false"])

            self.lite.guild_blacklist.add(target.id)
            await target.leave()
